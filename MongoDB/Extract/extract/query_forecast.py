...
def main():
	...
	for query in queries:
		...
		with pymongo.MongoClient(connection, connect=True) as c:
			ensemble_ids = np.array(sorted(c[database][collection].distinct("ensembleId", aggregate[0]["$match"])), dtype=str)
			for ensemble_id in ensemble_ids:
				location_ids = np.array(sorted(c[database][collection].distinct("locationId", {**aggregate[0]["$match"], "ensembleId": str(ensemble_id)})), dtype=str)
				ensemble_member_ids = np.array(sorted(c[database][collection].distinct("ensembleMemberId", {**aggregate[0]["$match"], "ensembleId": str(ensemble_id)})), dtype=str)

				first = c[database][collection].with_options(codec_options=CodecOptions(datetime_conversion=DatetimeConversion.DATETIME_CLAMP)).find_one({**aggregate[0]["$match"], "ensembleId": str(ensemble_id)}, {"metaData.timeStepMinutes": 1, "metaData.localTimeZone": 1})
				...
				min_max = c[database][collection].with_options(codec_options=CodecOptions(datetime_conversion=DatetimeConversion.DATETIME_CLAMP)).aggregate([{"$match": {**aggregate[0]["$match"], "ensembleId": str(ensemble_id)}}, {"$group": {"_id": None, "min": {"$min": f"${forecast_time_key}"}, "max": {"$max": f"${forecast_time_key}"}}}]).next()
				...
				event_times = np.array([pd.date_range(forecast_time_start, forecast_time_start + timedelta(days=lead_time_days) - timedelta(minutes=time_step_minutes), freq=f"{time_step_minutes}min") for _ in forecast_times])

				path = utils.get_folder_path(folder_overrides, aggregate, ensemble_id, time_step, timezone, file_format)
				utils.ensure_path(path)
				parameters.extend([(connection, database, collection, batch_size, path, file_format, forecast_time_chunk_size, forecast_times, time_step_minutes, event_times, lead_times, location_ids, location_id, ensemble_id, ensemble_ids, ensemble_member_ids, forecast_time_key, event_time_key, event_value_key, lead_time_days, aggregate, lock) for location_id in location_ids])
...
def run_query(connection, database, collection, batch_size, path, file_format, forecast_time_chunk_size, forecast_times, time_step_minutes, event_times, lead_times, location_ids, location_id, ensemble_id, ensemble_ids, ensemble_member_ids, forecast_time_key, event_time_key, event_value_key, lead_time_days, aggregate, lock):
	...
	with pymongo.MongoClient(connection, connect=True) as c:
		meta_data = c[database][collection].with_options(codec_options=CodecOptions(datetime_conversion=DatetimeConversion.DATETIME_CLAMP)).find_one({**aggregate[0]["$match"], "locationId": str(location_id), "ensembleId": str(ensemble_id)}, {"metaData": 1})["metaData"]
		actual_forecast_times = sorted(c[database][collection].distinct(forecast_time_key, {**aggregate[0]["$match"], "locationId": str(location_id), "ensembleId": str(ensemble_id)}))
		...
		for forecast_time in np.split(actual_forecast_times, np.arange(batch_size, len(actual_forecast_times), batch_size)):
			for result in c[database][collection].with_options(codec_options=CodecOptions(datetime_conversion=DatetimeConversion.DATETIME_CLAMP)).aggregate([
				{"$match": {**aggregate[0]["$match"], forecast_time_key: {"$in": forecast_time.tolist()}, "locationId": str(location_id), "ensembleId": str(ensemble_id)}}, *aggregate[1:],
				{"$project": {"_id": 0, "moduleInstanceId": 1, "parameterId": 1, "qualifierId": 1, "encodedTimeStepId": 1, "locationId": 1, "ensembleId": 1, "ensembleMemberId": 1, forecast_time_key: 1, f"timeseries.{event_time_key}": 1, f"timeseries.{event_value_key}": 1}}]):
	...
	print(f"{path} -> {ensemble_id} {location_id}")
...
def write_file(ds, path, file_format, location_ids, location_id, lead_times, ensemble_id, ensemble_member_ids, forecast_time_chunk_size, lock):
	...
	if file_format.startswith("zarr"):
		encoding = {"chunks": (1, forecast_time_chunk_size, 1, len(ensemble_member_ids), len(lead_times))}
		with lock:
			utils.write_zarr(ds, path, file_format, encoding)
	elif file_format in ["nc", "netcdf"]:
		encoding = {"compression": "zlib", "complevel": 5, "chunksizes": (1, forecast_time_chunk_size, 1, len(ensemble_member_ids), len(lead_times))
		}
		utils.write_netcdf(ds, path, location_id, ensemble_id, encoding)
	else:
		utils.write_csv(ds, path, location_id, ensemble_id)
