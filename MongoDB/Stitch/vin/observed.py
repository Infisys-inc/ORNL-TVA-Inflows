...
def main():
	...
	measure_attributes = pd.read_csv("observed_measure_attributes.csv", sep='\t', index_col=['folder']).to_dict(orient='index')

	with open('observed.json', 'r') as r:
		observed = json.load(r)

	data_root = observed['data_root']

	for timestep in observed['timesteps']:
...
def read(data_root, folders, location, extension, timestep_hours, all_location_attributes, location_attributes, dam_attributes, upstream_dams_flow_folder, tributaries_flow_folder, measure_attributes, path, seen):
	observed_data = [d for d in [get_observed_data(data_root, folder, location, extension, next((v for k, v in measure_attributes.items() if re.match(k, folder)), {})) for folder in folders] if d is not None and d.name not in seen]
	upstream_data = []
	tributary_data = []

	if append_upstream_data:
		for upstream_dam in dam_attributes.get('UpstreamDams', '').split(','):
			data = get_observed_data(data_root, upstream_dams_flow_folder, upstream_dam, extension, next((v for k, v in dam_attributes.items() if re.match(k, upstream_dams_flow_folder)), {}))
			if data is not None:
	...
	[d.rename(f'{d.name}_[{t}-US-TRIB]') for t, d in [(tributary, get_observed_data(data_root, tributaries_flow_folder, tributary, extension, next((v for k, v in measure_attributes.items() if re.match(k, tributaries_flow_folder)), {}))) for tributary in dam_attributes.get('Tributaries', '').split(',')] if d is not None]

	if observed_data:
		combined = combine(observed_data + upstream_data + tributary_data)
		write(combined, location, extension, timestep_hours, location_attributes, dam_attributes, path)
		print(location)
	return location, {n.name for n in observed_data}


def get_observed_data(data_root, folder, location, extension, measure_attributes):
...
def write(combined, location, extension, timestep_hours, location_attributes, dam_attributes, path):
	combined = format_netcdf(combined, location, timestep_hours, location_attributes, dam_attributes)
	encoding = {
