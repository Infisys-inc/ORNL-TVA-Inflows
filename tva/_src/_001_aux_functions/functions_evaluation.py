...
def calculate_nse(df, obs_col='y_obs', sim_col='y_sim'):
    """
    Calculate Nash-Sutcliffe Efficiency (NSE) between observed and simulated values.
    
    Parameters:
    - df: pandas DataFrame containing observed and simulated values
    - obs_col: column name for observed values (default 'y_obs')
    - sim_col: column name for simulated values (default 'y_sim')
    
    Returns:
    - NSE value (float)
    """
    # Extract observed and simulated values, dropping NaN pairs
    obs = df[obs_col].values
    sim = df[sim_col].values
    
    # Filter out positions where either obs or sim is NaN
